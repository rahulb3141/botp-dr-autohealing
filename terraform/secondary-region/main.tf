...
resource "aws_iam_role" "eks_cluster" {
  name = "${var.project_name}-secondary-eks-cluster-role"
...
resource "aws_iam_role" "eks_node" {
  name = "${var.project_name}-secondary-eks-node-role"
...
# EKS Cluster (Pilot Light - smaller configuration)
...
# EKS Node Group (Pilot Light - minimal nodes)
resource "aws_eks_node_group" "secondary_nodes" {
  cluster_name    = aws_eks_cluster.secondary.name
  node_group_name = "${var.project_name}-secondary-nodes"
  node_role_arn   = aws_iam_role.eks_node.arn
  subnet_ids      = aws_subnet.private[*].id

  scaling_config {
    desired_size = var.node_desired_size_dr
    max_size     = var.node_max_size
    min_size     = var.node_min_size_dr
  }
